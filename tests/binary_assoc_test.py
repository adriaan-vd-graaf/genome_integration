...
import genome_integration.association.c_binary_files
...
def make_binary_file_and_corresponding_bim():
    ...
    with gzip.open("test_resources/binary_file_test.dat.gz", "wb") as f:
    ...
    all_alleles = ['A', 'T', 'C', 'G']
    with open("test_resources/test_bim.bim", "w") as f:
        for i in range(num_snps):
...
def test_reading_binary_file():
    ...
    a = association.read_bin_file("tests/test_resources/binary_file_test.dat.gz", bim_file)
    end = time.time()
    print(end - start)
    ...
    # if this runs the associations are normal.
    bim_file = variants.BimFile("tests/test_resources/test_bim.bim")
    a = genome_integration.association.c_binary_files.read_bin_file("tests/test_resources/binary_file_test.dat.gz", bim_file)
    end = time.time()
    print(end - start)
